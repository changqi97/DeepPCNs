...
def stoppable_optimizer(base_optimizer: optax.GradientTransformation) -> optax.GradientTransformation:
    def init_fn(params):
        return base_optimizer.init(params)

    def update_fn(updates, state, params=None):
        mask = jax.tree_util.tree_map(
            lambda g: jnp.any(g != 0) if g is not None else False, updates
        )
        inner_updates, inner_new_state = base_optimizer.update(updates, state, params)

        final_updates = jax.tree_util.tree_map(
            lambda m, u: jnp.where(m, u, jnp.zeros_like(u)), mask, inner_updates
        )

        def _select_state(new_s, old_s):
            try:
                return jax.tree_util.tree_map(
                    lambda m, n, o: jnp.where(m, n, o), mask, new_s, old_s
                )
            except (ValueError, TypeError):
                return new_s

        final_state = jax.tree_util.tree_map(_select_state, inner_new_state, state)
        return final_updates, final_state
...
def train_on_batch(T: int, x: jax.Array, y: jax.Array, *, model, optim_w: pxu.Optim, optim_h: pxu.Optim, beta: float = 1.0):
    ...
    for t in range(T):
        ...
        with pxu.step(model, clear_params=pxc.VodeParam.Cache):
            mask = pxu.M(
                pxu.M_hasnot(pxc.VodeParam, frozen=True) | pxu.M(pxnn.LayerParam)
            ).to([False, True])(model, is_pytree=True)
...
def main(run_info: stune.RunInfo, save_model: bool, savepath: str = None, seed_idx: int = 0):
    ...
    schedule = optax.warmup_cosine_decay_schedule(
        ...
        warmup_steps=0.1 * len(train_dataloader) * nm_epochs * run_info["hp/T"],
        decay_steps=len(train_dataloader) * nm_epochs * run_info["hp/T"],
    ...
    optim_w = pxu.Optim(
        lambda: stoppable_optimizer(optax.adamw(schedule, weight_decay=run_info["hp/optim/w/wd"])),
    ...
    for e in range(50):
        train_time = train(train_dataloader, T=run_info["hp/T"], model=model,
                           optim_w=optim_w, optim_h=optim_h, beta=1.0)
        ...
        if a > best_accuracy:
            ...
            best_accuracy = a
            if save_model:
                save_name = f"./weights/{model_name}_{dataset_name}_{run_info['hp/precision_type']}"
